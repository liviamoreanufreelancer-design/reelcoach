    ...
    <PhoneShell>
      ...
      <div className="relative z-10 flex flex-col h-full px-7 pt-14 pb-10">
            ...
            {list.map((_, idx) => (
              <span
                ...
                className={`h-[3px] rounded-full transition-all duration-500 ${
                  idx === i ? "w-8 bg-gold-gradient" : "w-4 bg-white/25"
                }`}
              />
            ))}
          </div>
        ...
        {prof && (
          ...
          >
            <span className="w-7 h-7 rounded-full bg-white/[0.04] border border-white/10 flex items-center justify-center text-gold">
              <ProfessionIcon id={prof.id} className="w-4 h-4" />
            </span>
            <span className="text-[12px] text-white/90 font-medium">{prof.label}</span>
            <span className="text-[10px] tracking-[0.25em] uppercase text-gold-gradient font-semibold">schimbă</span>
          </button>
        )}

        <p className="mt-8 text-xs tracking-[0.4em] uppercase font-semibold text-gold-gradient">
          {t.tag}
        </p>
        ...
        <div className="mt-auto">
            ...
            {t.label.split(" / ").map((w, idx, arr) => (
              <span key={idx}>
                {idx === arr.length - 1 && arr.length > 1 ? (
                  <em className="italic font-editorial text-gold-gradient not-italic-fix">{w}</em>
                ) : (
                  <span>{w}</span>
                )}
                {idx < arr.length - 1 && <span className="text-gold/60"> / </span>}
              </span>
            ))}
          </h2>
          <p
          ...
          <div className="mt-8 flex items-center gap-3">
            ...
            <button
              ...
              className="flex-1 h-14 rounded-full bg-gold-gradient text-black font-semibold text-base shadow-gold active:scale-[0.98] transition"
            >
              Vezi ideile
            </button>
