export type FilterId =
  | "none"
  | "warm"
  | "cool"
  | "bw"
  | "vintage"
  | "vivid"
  | "cinema";

export interface FilterPreset {
  id: FilterId;
  label: string;
  desc: string;
  /** CSS filter string applied when drawing each video frame. */
  cssFilter: string;
  /** Optional rgba tint drawn on top with given alpha. */
  tint?: { color: string; alpha: number };
  /** Subtle vignette opacity 0..1. */
  vignette?: number;
}

export const FILTERS: Record<FilterId, FilterPreset> = {
  none: {
    id: "none",
    label: "Original",
    desc: "Fără filtru. Claritate maximă.",
    cssFilter: "none",
  },
  warm: {
    id: "warm",
    label: "Cald",
    desc: "Tonuri aurii, pielea arată natural.",
    cssFilter: "saturate(1.08) contrast(1.05) brightness(1.02)",
    tint: { color: "rgba(255, 170, 90, 1)", alpha: 0.06 },
  },
  cool: {
    id: "cool",
    label: "Rece",
    desc: "Albastru editorial, look modern.",
    cssFilter: "saturate(1.05) contrast(1.06)",
    tint: { color: "rgba(110, 160, 220, 1)", alpha: 0.07 },
  },
  bw: {
    id: "bw",
    label: "Alb-Negru",
    desc: "Monocrom elegant, contrast crescut.",
    cssFilter: "grayscale(1) contrast(1.12) brightness(1.02)",
  },
  vintage: {
    id: "vintage",
    label: "Vintage",
    desc: "Sepia ușor, nostalgie discretă.",
    cssFilter: "sepia(0.35) saturate(1.1) contrast(1.05)",
    tint: { color: "rgba(190, 140, 80, 1)", alpha: 0.05 },
    vignette: 0.25,
  },
  vivid: {
    id: "vivid",
    label: "Vivid",
    desc: "Culori vii, pop pentru produse.",
    cssFilter: "saturate(1.25) contrast(1.1) brightness(1.02)",
  },
  cinema: {
    id: "cinema",
    label: "Cinema",
    desc: "Teal & orange subtil, look de film.",
    cssFilter: "saturate(1.12) contrast(1.1)",
    tint: { color: "rgba(255, 140, 70, 1)", alpha: 0.05 },
    vignette: 0.3,
  },
};

export const FILTER_LIST: FilterPreset[] = [
  FILTERS.none,
  FILTERS.warm,
  FILTERS.cool,
  FILTERS.cinema,
  FILTERS.vintage,
  FILTERS.bw,
  FILTERS.vivid,
];
